...
const SOCKET_URL = (import.meta.env.VITE_SOCKET_URL || import.meta.env.VITE_API_URL || "http://localhost:4000").replace(/\/api$/, "");
...
export default function Dashboard() {
  ...
  const [widgetConfig, setWidgetConfig] = useState(() => {
    const saved = typeof window !== "undefined" ? localStorage.getItem("seo_dashboard_widgets") : null;
    if (saved) {
      try { return JSON.parse(saved); } catch {}
    }
    return { showTraffic: true, showTimeline: true, showVuln: true, showAlerts: true };
  });

  const fetchAll = async () => {
    try {
      setLoading(true);
      const [logsRes, trafficRes, vulnRes, alertsRes, scansRes] = await Promise.all([
        api.get("/logs/stats"),
        api.get("/traffic/stats"),
        api.get("/vulnerabilities/stats"),
        api.get("/logs/alerts", { params: { severity: "all", acknowledged: "false" } }),
  ...
  useEffect(() => {
    ...
    socket.on("new-alert", (p) => {
      setRecentAlerts((prev) => {
        ...
        const n = { _id: p.id, severity: p.severity, title: p.title, description: p.description, createdAt: p.createdAt };
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      localStorage.setItem("seo_dashboard_widgets", JSON.stringify(widgetConfig));
    }
  }, [widgetConfig]);

  const toggleWidget = (k) => setWidgetConfig((p) => ({ ...p, [k]: !p[k] }));

  const attackTimelineData = useMemo(() => {
    if (!trafficStats?.recentSpikes) return [];
    const m = new Map();
    trafficStats.recentSpikes.forEach((i) => {
      const ts = new Date(i.createdAt);
      if (isNaN(ts)) return;
      const key = `${ts.getHours().toString().padStart(2, "0")}:${Math.floor(ts.getMinutes() / 5).toString().padStart(2, "0")}`;
      m.set(key, (m.get(key) || 0) + 1);
    });
    return [...m.entries()].map(([t, c]) => ({ time: t, spikes: c })).sort((a, b) => (a.time > b.time ? 1 : -1));
  }, [trafficStats]);

  const trafficByMethodData = useMemo(() => {
    if (!trafficStats?.byMethod) return [];
    return trafficStats.byMethod.map((m) => ({ method: m._id, count: m.count }));
  }, [trafficStats]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-black via-slate-950 to-slate-900 text-gray-100">
      ...
      <div className="w-full mx-auto px-3 sm:px-4 lg:px-6 py-6 space-y-6" style={{ maxWidth: 'min(1400px, calc(100vw - 32px))' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <MetricCard icon={<AlertOctagon className="w-4 h-4" />} label="Open Vulns" value={vulnStats?.status?.open ?? 0} chip={`${vulnStats?.severity?.critical || 0} critical • ${vulnStats?.severity?.high || 0} high`} gradient="from-rose-500/30 via-orange-500/40 to-amber-400/20" border="border-rose-500/40" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="space-y-4">
                  ...
                  <div className="w-1/2 h-full">
                    {vulnSeverityData.length ? (
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          <Pie data={vulnSeverityData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={70} innerRadius={40} paddingAngle={3}>
                            {vulnSeverityData.map((e, i) => (
                              <Cell key={e.name} fill={SEVERITY_COLORS[e.name] || PIE_COLORS[i % PIE_COLORS.length]} />
                            ))}
                          </Pie>
            ...
            {widgetConfig.showTraffic && (
              <PanelShell icon={<LineChart className="w-4 h-4 text-cyan-300" />} title="Traffic Overview" subtitle="Requests and methods" accent="from-cyan-500/30 via-sky-500/30 to-purple-500/30">
                <div className="grid grid-cols-1 gap-3 h-64">
                  <div className="h-32">
                    {trafficStats ? (
                      <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={[{ label: "Now", requests: trafficStats.total || 0, uniqueIps: trafficStats.uniqueIps || 0 }]}>
                          <defs>
                          ...
                          <Tooltip contentStyle={{ backgroundColor: "#020617", borderRadius: 8, border: "1px solid rgba(148,163,184,0.4)", fontSize: 12 }} labelStyle={{ color: "#e5e7eb" }} />
                          <Area type="monotone" dataKey="requests" stroke="#22d3ee" fill="url(#trafficRequests)" strokeWidth={2} name="Total Requests" />
                        </AreaChart>
                      </ResponsiveContainer>
                    ) : (
                      <EmptyChartMessage small />
                    ...
                    {trafficByMethodData.length ? (
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={trafficByMethodData}>
                          ...
                          <YAxis tick={{ fill: "#9ca3af", fontSize: 11 }} />
                          <Tooltip contentStyle={{ backgroundColor: "#020617", borderRadius: 8, border: "1px solid rgba(148,163,184,0.4)", fontSize: 12 }} />
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {widgetConfig.showTimeline && (
              <PanelShell icon={<Radar className="w-4 h-4 text-amber-300" />} title="Attack Timeline" subtitle="Spike events" accent="from-amber-400/30 via-orange-500/30 to-rose-500/30">
                <div className="h-64">
                  {attackTimelineData.length ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={attackTimelineData}>
                        ...
                        <Tooltip contentStyle={{ backgroundColor: "#020617", borderRadius: 8, border: "1px solid rgba(148,163,184,0.4)", fontSize: 12 }} />
                        <Area type="monotone" dataKey="spikes" stroke="#fb923c" fill="url(#attackSpikes)" strokeWidth={2} name="Spike Events" />
                      </AreaChart>
                    </ResponsiveContainer>
                  ) : (
            ...
            {widgetConfig.showAlerts && (
              <PanelShell icon={<Shield className="w-4 h-4 text-cyan-300" />} title="Recent Scans" subtitle="Scanner history" accent="from-cyan-500/30 via-blue-500/30 to-slate-700/30">
                <div className="space-y-2 max-h-52 overflow-y-auto custom-scroll">
                  {!recentScans.length ? (
                    ...
                    recentScans.slice(0, 6).map((s) => (
                      <div key={s._id || s.scanId} className="flex justify-between items-center text-xs px-2 py-2 rounded-lg bg-slate-900/70 border border-slate-800/80">
                        <div className="space-y-0.5">
                          <p className="text-slate-100 truncate max-w-[170px]">{s.siteUrl}</p>
                          <p className="text-[11px] text-slate-500">
                            {s.status} • {s.startedAt ? new Date(s.startedAt).toLocaleString() : ""}
                          </p>
                        </div>
            ...
            <div className="rounded-2xl border border-slate-800/80 bg-gradient-to-br from-slate-950/80 via-slate-900/80 to-slate-950/80 px-4 py-3 text-[11px] text-slate-300 space-y-2">
              <div className="flex items-center gap-2">
                ...
                <span className="uppercase tracking-[0.2em] text-slate-400">Widget Layout</span>
              </div>

              <div className="grid grid-cols-2 gap-2 pt-1">
                <WidgetToggle label="Traffic" checked={widgetConfig.showTraffic} onChange={() => toggleWidget("showTraffic")} />
                <WidgetToggle label="Timeline" checked={widgetConfig.showTimeline} onChange={() => toggleWidget("showTimeline")} />
                <WidgetToggle label="Vulnerabilities" checked={widgetConfig.showVuln} onChange={() => toggleWidget("showVuln")} />
                <WidgetToggle label="Scan History" checked={widgetConfig.showAlerts} onChange={() => toggleWidget("showAlerts")} />
              </div>
            </div>
          </div>
        </div>
...
function EmptyChartMessage({ small, message }) {
  return (
    <div className={`flex flex-col items-center justify-center text-center ${small ? "h-full" : "h-full"}`}>
      <div className="p-2 rounded-xl bg-slate-900/80 border border-slate-800/80 mb-2">
        <ServerCrash className="w-4 h-4 text-slate-500" />
      </div>
      <p className="text-[11px] text-slate-400">{message || "Not enough data"}</p>
    </div>
  );
}
...
function HudPanel({ logStats, trafficStats, vulnStats, alertsCount }) {
  ...
  const threatScore = (openVulns * 6) + (alertsCount * 4) + ((vulnStats?.severity?.critical ?? 0) * 10);
      ...
      <div className="relative flex justify-center my-6">
        <div className={`relative w-48 h-48 rounded-full border-2 ${ring} ${glow} transition-all duration-500`}>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <p className="text-[11px] text-slate-400">{alertsCount} Alerts • {openVulns} Vulns</p>
          </div>
        </div>
      </div>
